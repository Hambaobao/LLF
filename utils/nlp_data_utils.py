...
class InputFeatures(object):
    ...
    def __init__(self, input_ids=None, input_mask=None, segment_ids=None, tokens_term_ids=None, tokens_sentence_ids=None, term_input_ids=None, term_input_mask=None, term_segment_ids=None, sentence_input_ids=None, sentence_input_mask=None, sentence_segment_ids=None, tokens_term_sentence_ids=None, label_id=None, masked_lm_labels=None, masked_pos=None, masked_weights=None, position_ids=None, valid_ids=None, label_mask=None):
...
class DscProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, fn="train.json"):
        """See base class."""
        return self._create_examples(self._read_json(os.path.join(data_dir, fn)), "train")

    def get_dev_examples(self, data_dir, fn="dev.json"):
        """See base class."""
        return self._create_examples(self._read_json(os.path.join(data_dir, fn)), "dev")

    def get_test_examples(self, data_dir, fn="test.json"):
        """See base class."""
        return self._create_examples(self._read_json(os.path.join(data_dir, fn)), "test")
...
def convert_examples_to_features(config, examples, label_list, max_seq_length, tokenizer, mode):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if config.exp in ['3layer_aspect', '2layer_aspect_transfer', '2layer_aspect_dynamic']:
            term_position = tokens.index('[SEP]') - 1
            while term_position < config.max_term_length:  #[CLS],t,[SEP]
...
#         features.append(InputFeatures(tokens_term_ids=tokens_a_ids, tokens_sentence_ids=tokens_a_ids, label_id=label_id))
#     return features
...
#         features.append(InputFeatures(tokens_term_ids=tokens_a_ids, tokens_sentence_ids=tokens_b_ids, label_id=label_id))
...
#         tokens_ids = tokenizer.texts_to_sequences([example.text_a.translate(str.maketrans('', '', string.punctuation)).lower() + ' ' + example.text_b.translate(str.maketrans('', '', string.punctuation)).lower()])
...
#         features.append(InputFeatures(tokens_term_sentence_ids=tokens_ids, tokens_term_ids=tokens_a_ids, tokens_sentence_ids=tokens_b_ids, label_id=label_id))
...
def convert_examples_to_features_dtc(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features.append(InputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids, label_id=labels_a))
